      ...
      (class* cls (externalizable<%>)
        ...
        (define/public (internalize str)
          (let* ([jsx (string->jsexpr str)]
                 ...
                 [col-defs (get-class-metadata columns cls)])
...
(define-syntax-rule (xml-data-class-mixin cls)
  (if (data-class? cls)
      (class* cls (externalizable<%>)
        (define/public (externalize)
          (let-values ([(cls cls-id-key st-key tbl-nm col-defs j-defs pkey auto-key ext-nm) (data-class-info cls)]
                       [(cls-nm fld-cnt fld-nms fld-acc fld-mut sup-cls skpd?) (class-info cls)])
            (xexpr->string 
             (append (list (string->symbol ext-nm) '()) 
                     (map (lambda (d) (list (string->symbol (third d)) '() 
                                            (~a (dynamic-get-field (first d) this)))) (get-class-metadata columns cls))))))
        (define/public (internalize str)
